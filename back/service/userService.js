...
const service = {
  // 중복 검사
  async duplicationCheck(params) {
    let duplicationUser = null;

    try {
      duplicationUser = await userDao.selectUser(params);
      logger.debug(
        `(userService.duplicationCheck) ${JSON.stringify(duplicationUser)}`
      );

      // UserId 중복
      if (duplicationUser) {
        const err = new Error("Not allowed duplicationUser");
      ...
      logger.error(`(userService.duplicationCheck) ${err.toString()}`);
      return new Promise((resolve, reject) => {
        reject(err);
      });
    }
  ...
  async reg(params) {
    ...
    const newParams = {
      ...params,
      password: hashPassword, // password 값 덮어쓰기
    };
    try {
